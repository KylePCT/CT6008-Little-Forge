    ...
    SubShader {
        ...
        Pass {
            ...
            UNITY_INSTANCING_BUFFER_START( Props )
                ...
                UNITY_DEFINE_INSTANCED_PROP( float4, _FireMainColour)
                UNITY_DEFINE_INSTANCED_PROP( float, _DistortionIntensity)
                UNITY_DEFINE_INSTANCED_PROP( float4, _SupportColour)
            ...
            VertexOutput vert (VertexInput v) {
                ...
                v.vertex.xyz += float3(_Metallic_var,_Metallic_var,_Metallic_var);
            ...
            float4 frag(VertexOutput i, float facing : VFACE) : COLOR {
                ...
                float4 node_1119 = _Time;
                float4 node_5912 = _Time;
                float2 node_6049 = ((i.uv0+node_5912.r)+node_1119.g*float2(0.1,0.4));
                ...
                float node_4465 = saturate(( _OpacityMask_var.rgb > 0.5 ? (_NoiseTexture_var.rgb/((1.0-_OpacityMask_var.rgb)*2.0)) : (1.0-(((1.0-_NoiseTexture_var.rgb)*0.5)/_OpacityMask_var.rgb)))).r;
                clip(node_4465 - 0.5);
...
/////// Diffuse:
                ...
                float4 _FireMainColour_var = UNITY_ACCESS_INSTANCED_PROP( Props, _FireMainColour );
                float4 _SupportColour_var = UNITY_ACCESS_INSTANCED_PROP( Props, _SupportColour );
                float _GradientStrength_var = UNITY_ACCESS_INSTANCED_PROP( Props, _GradientStrength );
                float4 _FireTexture_var = tex2D(_FireTexture,TRANSFORM_TEX(node_3725, _FireTexture));
                float3 node_4928 = (lerp(_FireMainColour_var.rgb,_SupportColour_var.rgb,(i.uv0.g*_GradientStrength_var))*_FireTexture_var.rgb);
                indirectDiffuse += node_4928; // Diffuse Ambient Light
...
/// Final Color:
                ...
                fixed4 finalRGBA = fixed4(finalColor,node_4465);
        ...
        Pass {
            ...
            float4 frag(VertexOutput i, float facing : VFACE) : COLOR {
                ...
                float3 viewDirection = normalize(_WorldSpaceCameraPos.xyz - i.posWorld.xyz);
                float3 normalDirection = i.normalDir;
                float4 node_1761 = _Time;
                float4 node_5912 = _Time;
                float2 node_6049 = ((i.uv0+node_5912.r)+node_1761.g*float2(0.1,0.4));
                float4 _NoiseTexture_var = tex2D(_NoiseTexture,TRANSFORM_TEX(node_6049, _NoiseTexture));
                float _DistortionIntensity_var = UNITY_ACCESS_INSTANCED_PROP( Props, _DistortionIntensity );
                float2 node_3725 = lerp(_NoiseTexture_var.rgb,float3(i.uv0,0.0),_DistortionIntensity_var).rg;
                float4 _OpacityMask_var = tex2D(_OpacityMask,TRANSFORM_TEX(node_3725, _OpacityMask));
...
/// Final Color:
                ...
                fixed4 finalRGBA = fixed4(finalColor * node_4465,0);
                UNITY_APPLY_FOG(i.fogCoord, finalRGBA);
                return finalRGBA;
            }
            ENDCG
        ...
        Pass {
            ...
            float4 frag(VertexOutput i, float facing : VFACE) : COLOR {
                ...
                float faceSign = ( facing >= 0 ? 1 : -1 );
                float3 viewDirection = normalize(_WorldSpaceCameraPos.xyz - i.posWorld.xyz);
                float4 node_1439 = _Time;
                float4 node_5912 = _Time;
                float2 node_6049 = ((i.uv0+node_5912.r)+node_1439.g*float2(0.1,0.4));
                float4 _NoiseTexture_var = tex2D(_NoiseTexture,TRANSFORM_TEX(node_6049, _NoiseTexture));
                float _DistortionIntensity_var = UNITY_ACCESS_INSTANCED_PROP( Props, _DistortionIntensity );
                float2 node_3725 = lerp(_NoiseTexture_var.rgb,float3(i.uv0,0.0),_DistortionIntensity_var).rg;
                float4 _OpacityMask_var = tex2D(_OpacityMask,TRANSFORM_TEX(node_3725, _OpacityMask));
        ...
        Pass {
            ...
            float4 frag(VertexOutput i, float facing : VFACE) : SV_Target {
                ...
                float4 node_5912 = _Time;
                float2 node_6049 = ((i.uv0+node_5912.r)+node_3943.g*float2(0.1,0.4));
                float4 _NoiseTexture_var = tex2D(_NoiseTexture,TRANSFORM_TEX(node_6049, _NoiseTexture));
                float _DistortionIntensity_var = UNITY_ACCESS_INSTANCED_PROP( Props, _DistortionIntensity );
                float2 node_3725 = lerp(_NoiseTexture_var.rgb,float3(i.uv0,0.0),_DistortionIntensity_var).rg;
                float4 _FireTexture_var = tex2D(_FireTexture,TRANSFORM_TEX(node_3725, _FireTexture));
